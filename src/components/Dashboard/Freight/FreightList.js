...
class FreightList extends React.Component {
  ...
  async UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (this.state.APIAction == 3) {
      ...
      if (nextProps.FreightList.GetFreightListSuccess) {
  ...
  async FetchFreightList(isSearchAction, searchTxt = "") {
    ...
    await this.props.GetFreightListAction(searchTxt, 0, this.state.PageCount);
  }
  ...
  async handleFilterSubmit(FilterOptions = null) {
    ...
    this.setState({ APIAction: 0 });
    await this.props.GetFreightListAction(
      "",
      0,
